...
def get_thumbnail(data):
    return change_image_to_high_res(safe_get(data, 6, 72, 0, 1, 6, 0))
...
def get_images(data):
    ...
    for element in images:
        ...
        if thumbnail:
            ls.append({"about": title, "link": change_image_to_high_res(thumbnail)})
    return ls
...
def get_is_temporarily_closed(data, status):
    ...
    if items is not None:
        for item in items:
            testi = item[-2][-1]
            if "place/rap/changeexistence/reopen" in testi:
                return True
    else:
        # TODO: fix when this occurs and resolve that issue, Nationwide Bank Baker Street
        pass
    return False
